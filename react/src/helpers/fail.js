...
export default function fail({apprentice, mentor, questTitle, node1, node2, node3, node4, node5, dateStart, dateEnd}, num_completed_nodes){
  ...
  let story = [ `In a distant land, far beyond yonder, a noble adventure who goes by the name of ${apprentice} sets off on a journey to ${questTitle.toLowerCase()}. With their trusted mentor, ${mentor} by their side, the odds were in their favour.`];
  ...
  const fail_line = `It appeared as if lady luck had turned a blind eye to their fate, however. The expected became the unexpected and it was soon clear to ${mentor} that it was simply not the right time for ${apprentice} to ${questTitle.toLowerCase()}. Without any delay, ${mentor} broke the news to his pupil. It was a rough for ${apprentice} to face the music but ${apprentice} accepted the reality of thesituation. “Fear not. Take as much time as you need to vent but do not dwell forever in today’s failure. Instead, take it as a marker... a marker for progressfrom where you started. For should you decide to partake this quest again, you would be ${experience} ahead. That my friend, is the meaning of experience.” A smile crept upon ${apprentice}’s face as he heeded his mentor’s words and looked forward to the day he decides to ${questTitle.toLowerCase()} again.`;

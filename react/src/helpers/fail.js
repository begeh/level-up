...
export default function fail({ apprentice, mentor, questTitle, nodes, dateStart, dateEnd }, num_completed_nodes, user_id, mentor_id) {
  ...
  if (nodes.length === 5) {
    if (mentor_id === user_id) {
      ...
      story = [`In a distant land, far beyond yonder, a noble adventurer who goes by the name of ${apprentice} sets off on a journey to ${questTitle.toLowerCase()}. `];
      ...
      story_parts = [
        ...
        `“${nodes[2].title.charAt(0) + nodes[2].title.slice(1).toLowerCase()}?”, said ${apprentice}. "That can prove quite tricky.”`, 
        
        `“I should take this opportunity to catch my breath and plan ahead because my next task is to ${nodes[3].title.toLowerCase()}.”
        
        Hence, ${apprentice} used the extra time to prepare for the next task.`,
      ...
      fail_line = ` It appeared as if lady luck had turned a blind eye to ${apprentice}’s fate. The expected became the unexpected and it was soon clear to ${apprentice} that it was simply not the right time to complete the quest. It was a rough for ${apprentice} to face the music but they accepted the reality of the situation. ${apprentice} took this experience as a marker... a marker of progress from when they had started. For should ${apprentice} decide to partake in this quest again, they will be ${experience} ahead. Until then, ${apprentice} will take as much time as they need to reflect on the journey they have just taken.`;

    } else {

      story = [`In a distant land, far beyond yonder, a noble adventurer who goes by the name of ${apprentice} sets off on a journey to ${questTitle.toLowerCase()}. With their trusted mentor ${mentor} by their side, the odds were in their favour.`];
